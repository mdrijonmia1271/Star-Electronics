...
class Salary extends Admin_Controller {
    ...
    public function salary_sheet($emit = NULL) {
     ...
     public function bonus($emit = NULL) {
        ...
        $this->data['confirmation'] = $this->data['employee'] = null;
        ...
        if ($this->input->post('create')) {
         ...
         $this->data['percent'] = $this->action->read('bonus');
    ...
    public function set_bonus(){
        ...
        $date = explode("/",$date);
         ...
         $data=array(
             ...
             'year'          => $date[1],
             'month'         => $date[0],
             'bonus_percent' => $this->input->post('bonus')
           );
        ...
        $this->data['confirmation']=message($this->action->add('bonus',$data),$msg_array);                 
      ...
      $this->data['confirmation'] = null;     
       ...
       $msg_array=array(
        ...
        $this->data['confirmation']=message($this->action->deleteData('bonus',array('id'=>$id)),$msg_array);
        $this->session->set_flashdata('confirmation',$this->data['confirmation']);
        redirect('salary/salary/bonus','refresh');
    ...
    public function edit_bonus($id=NULL) {
        ...
        $this->data['percent'] = $this->action->read('bonus',array('id'=>$id));
        $this->data['employee'] = $this->action->readGroupBy("employee","emp_id",array(),"id","asc");
    ...
    public function bonus_edit($id=NULL) {  
        ...
        $this->data['confirmation']=message($this->action->update('bonus', $data, array('id'=>$id)), $msg_array);
        $this->session->set_flashdata('confirmation',$this->data['confirmation']);
        redirect('salary/salary/bonus','refresh');
    }
  ...
  public function advanced() {
        ...
        // work with DB
        if($this->input->post('create')) {
            $date = ($this->input->post("payment_date")) ? $this->input->post("payment_date") : date("Y-m-d");
            ...
            $this->session->set_flashdata('confirmation', message($this->action->add("advanced_payment",$data), $options));
         ...
         if($this->input->get('id') != "") {
            ...
            $this->session->set_flashdata('confirmation', message($this->action->deleteData("advanced_payment",$where), $options));
            redirect('salary/salary/advanced', 'refresh');
        }
